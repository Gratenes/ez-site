...
import type { twitterFetch } from "@/utils/twitter";
...
function EnlargeCard({ data }: { data: twitterMedia }) {
...
export default (preload: any) => {
  const { data }: { data: twitterFetch } = preload;
    ...
    if (num) {
      ...
      media.forEach((doc, index) => {
        setTimeout(async () => {
          ...
          const anchor = document.createElement("a");
          anchor.href = URL.createObjectURL(blob);
          anchor.download = `${data.user.name}-${index}.${contentType?.split("/")[1]}`;
          anchor.style.display = "none";
          document.body.appendChild(anchor);
          anchor.click();
          document.body.removeChild(anchor);
        ...
        >
          <motion.div layout className="w-fit rounded-[26px] border border-solid border-white overflow-clip flex flex-col md:flex-row">
            ...
            <motion.div layout id="video+stats">
              ...
              >
                {data?.content?.media?.map((entry: twitterMedia, index) =>
